...
//console.log(postCategory)//data in data
 ...
 <div className={`  ${isOpen ? 'block' :'hidden'}`} >
  {postCategory.data.map((item, index)=>{
    return <Link href={`/blogs/${item.englishTitle}`} className='block mb-1 py-4 px-4 hover:bg-purple-50'>{item.title
    } </Link>
  })}
      ...
      <div class="md:hidden flex gap-5 overflow-auto pb-5">
      {postCategory.data.map((item, index)=>{
    return <Link href={`/blogs/${item.englishTitle}`} className='border border-gray-500 hover:bg-purple-50 text-gray-500 text-sm py-1 px-5 rounded-3xl'>{item.title
    } </Link>
    })}
      </div>
      </>
